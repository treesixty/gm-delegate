...
| ~~`CombatTracker` (v14) has a protected `_getEntryContextOptions()` method...~~ **RESOLVED 2026-08-12, and the guess was wrong.** Live-inspected the actual method bodies via the browser console (`ui.combat`'s prototype chain, `.toString()` on each override) rather than TypeDoc: core `CombatTracker.prototype._getEntryContextOptions()` returns a **hardcoded array literal** — no `Hooks.call`/`Hooks.callAll` anywhere in it. The `getCombatTrackerEntryContext` hook does not exist in v14; `main.js`'s registration for it was dead code (confirmed live: an unfiltered `Hooks.callAll` spy logged nothing context/combat-related on right-click). The option-object shape was also wrong (`name`/`condition`/`callback`, not the real `label`/`icon`/`visible`/`onClick`). Fixed by patching `_getEntryContextOptions` on the base class directly, resolved via `ui.combat`'s own prototype chain rather than a guessed import path (dnd5e's subclass calls `super._getEntryContextOptions()`, so the patch is picked up live). | 2026-08-12 | live console inspection, RunPod pod `d90mhv7i5kvqyg`, v14.365, dnd5e | Low |
...
| **TokenHUD wired to `voiceNpc()`. Resolved and shipped 2026-08-12.** `renderTokenHUD` confirmed live to fire on `canvas.hud.token.bind(token)`, handler args `(hud, html)` where `html` is a raw `HTMLFormElement` (not jQuery — v14 has moved this HUD off jQuery) and `hud.object` is the bound `Token` placeable (`hud.object.actor` gives the synthetic actor). Unlike the M3 `getCombatTrackerEntryContext` case, this hook is real and needed no prototype patch — a plain `Hooks.on` at `ready` is enough since the HUD only renders on demand. Injects a `.control-icon` button into `.col.right`, matching the existing convention. | 2026-08-12 | live console inspection, RunPod pod `d90mhv7i5kvqyg`, v14.365 | Low |
| **`Actor#hasPlayerOwner` confirmed live** (not found via TypeDoc's own `?q=` search — same tooling gap as the row-28 `canvas.level.id` false alarm; ground-truthed directly against a running instance instead). `actor.hasPlayerOwner` is a real boolean accessor, `false` for an NPC actor with no player-owned permission grant. Backs the `isNpcActor()` check (`panel.js`) per the user's definition: NPC = not a player-character and not player-controlled, so `actor.type !== "character" && !actor.hasPlayerOwner`. | 2026-08-12 | live console inspection, RunPod pod `d90mhv7i5kvqyg`, v14.365 | Low |
| **Adopted 2026-08-12** (was "Not adopted" as of the row below, now superseded — kept for the reasoning trail). Foundry exposes dedicated `combatTurn(combat, updateData: {round, turn}, updateOptions)`, `combatRound(combat, updateData, updateOptions)`, and `combatStart(combat, updateData)` hooks, confirmed by TypeDoc **and** live-tested against a real Combat on pod `d90mhv7i5kvqyg`: `startCombat()` fires only `combatStart`; a same-round `nextTurn()` fires only `combatTurn`; a round-wrapping `nextTurn()` fires only `combatRound`. Never more than one per advance, so `eventbus.js` maps all three to the single `combat.turn` event with no double-emit risk. All three fire **before** the database update, so `updateData.round`/`.turn` — not `combat.round`/`.turn` on the document — are what `eventbus.js` reads. §4.6's `HOOKS` list updated to match (spec change applied, not just proposed, per the user's 2026-08-12 sign-off). | 2026-08-12 | `foundryvtt.com/api/functions/hookEvents.combatTurn.html`; `.combatRound.html`; `.combatStart.html`; live console inspection, RunPod pod `d90mhv7i5kvqyg`, v14.365 | Low |
| `RollTable` also exposes `drawMany(number, {displayChat, messageMode, recursive, roll})`, resolving to `{roll, results}` for pulling multiple results at once (dice-pool style) — row above already covers single-result `draw()`. Neither is adopted yet; M6 hasn't started. | 2026-08-12 | `foundryvtt.com/api/classes/foundry.documents.RollTable.html` | Low |
| `foundry.applications.api.DialogV2` exists in v14: static `confirm()`/`prompt()`/`input()`/`wait()` helpers, `buttons: [{action, label, default, callback(event, button, dialog)}]`, content as HTML string/element/Handlebars output. Flagging as a candidate for M7's card chrome (currently a raw JSON dump per `docs/milestones/03-panel.md`'s "ugly is fine, ugly is honest") — not adopted, M7 hasn't started. | 2026-08-12 | `foundryvtt.com/api/classes/foundry.applications.api.DialogV2.html` | Low |
| `game.tables` is the confirmed world `RollTable` collection ("accessible within the Game object as game.tables"), backing `list_roll_tables` (§5.2) — filter by name via `.filter()`/`.getName()`. `CompendiumCollection#getDocument(id): Promise<TDocument>` ("Get a single Document from this Compendium by ID... may already be locally cached, otherwise retrieved from the server") backs `get_compendium_actor`'s explicit pack+ID lookup. Neither adopted yet, M6 hasn't started. | 2026-08-12 | `foundryvtt.com/api/classes/foundry.documents.collections.RollTables.html`; `foundryvtt.com/api/classes/foundry.documents.collections.CompendiumCollection.html` | Low |
| **§5.6 rule 3 corrected.** Browsers expose no API to listen for incoming TCP/WebSocket connections — only `new WebSocket(url)` to dial out. `module.json` declares only a client-side `esmodules` entry (`scripts/main.js`), so `scripts/socket.js` runs inside the GM's browser and cannot be the WS server the original spec text assigned it to be. The **agent** (a real Node process) binds the listening socket; the **module** dials out and reconnects. Not a Foundry-API fact so much as a basic platform constraint the original spec prose got backwards — recorded here per the existing convention of logging drift between spec prose and buildable reality. | 2026-08-12 | MDN `WebSocket` interface (no listen/accept method exists on it, by design — the browser platform, not a version-specific fact) | Low |
| **`TableResult` schema uses `documentUuid`, not the older `documentCollection`/`documentId` pair**, for linking a row to a compendium document — a single UUID string, resolvable via `fromUuid()` like everything else in this codebase. `place_encounter`'s creatures need a `packId`/`actorId` pair, not a UUID, so `rollOnTable` (`encounter.js`, M7) resolves it: `fromUuid(result.documentUuid)` then `doc.compendium.collection` (packId) and `doc.id` (actorId) — see the next two rows. A plain-text (non-linked) row has no `documentUuid`; handled defensively (nulls), not assumed present. **Still unconfirmed live**: the actual Thornwood table's rows are plain text (STATUS.md, 2026-08-12), so this path has no real input yet — re-authoring them as pack-linked is a live-session prerequisite, same class of gap as the original "author rows as `[[2d4]] Wolves`" requirement. | 2026-08-14 | `foundryvtt.com/api/classes/foundry.documents.TableResult.html` | Medium — re-verify once the Thornwood table is re-authored and a real pack-linked row is drawn |
| `Document#compendium` is a confirmed accessor ("A reference to the Compendium Collection containing this Document, if any, and otherwise null") — there is **no** separate `Document#pack` string getter in v14's documented API (unlike some older-version recall). `CompendiumCollection#collection` (`string`) is "the canonical Compendium name — comprised of the originating package and the pack name," the exact string `game.packs.get(id)` expects. So the pack id off a resolved document is `doc.compendium.collection`, not `doc.pack`. `encounter.js`'s `resolveCreatureLink()` (M7) uses this. | 2026-08-14 | `foundryvtt.com/api/classes/foundry.documents.Actor.html`; `foundryvtt.com/api/classes/foundry.documents.collections.CompendiumCollection.html` | Low |
| `foundry.applications.handlebars.loadTemplates(paths)` registers each loaded template as a Handlebars partial — by full path when `paths` is an array (`{{> "templates/apps/foo.html"}}`), or by the given key when `paths` is an object map (`{{> foo}}`). Backs `templates/card-encounter.hbs` being included from `panel.hbs` via `{{> card-encounter}}` (M7) — `main.js`'s `init` hook calls `loadTemplates({"card-encounter": "modules/gm-delegate/templates/card-encounter.hbs"})` before the panel ever renders. | 2026-08-14 | `foundryvtt.com/api/functions/foundry.applications.handlebars.loadTemplates.html` | Low |
...
**`combat.turn` is sourced from `combatStart`/`combatRound`/`combatTurn`, not `updateCombat`.** Adopted 2026-08-12 (spec §0) after `updateCombat` — which fires on *any* Combat document write, not just a turn advance — produced a noisy `{round: 2, turn: null}` on an empty-tracker advance. The three dedicated hooks fire only on an actual start/round/turn change and are confirmed mutually exclusive per advance (live-tested against a real Combat: `startCombat()` fires only `combatStart`; a same-round `nextTurn()` fires only `combatTurn`; a round-wrapping `nextTurn()` fires only `combatRound` — never more than one, so mapping all three to the single `combat.turn` event risks no double-emit). They fire *before* the database write, so `updateData.round`/`.turn` — not `combat.round`/`.turn` on the document — carry the values.
...
- Right-click an NPC token → "I'll voice this one" (writes `actorOverrides`). "NPC" = not a
  player-character and not player-controlled (`actor.type !== "character" && !actor.hasPlayerOwner`),
  not a bare type check — a player-owned hireling with an "npc" actor type is still theirs to voice.
...
**The complete type list for v1. Seven types. If it is not here, it does not exist.**
...
| `TRIGGER` | module → agent | `{ text }`, the §4.7 panel's GM-command input | none in v1, fire-and-forget. Added M7 (STATUS.md) — its own type rather than an `EVENT`, since a GM-authored command isn't hook telemetry and shouldn't be forced through `EVENT`'s "must match the HOOKS table" contract. What it kicks off (a `20_resolve` → `30_scene` run) arrives later as the agent's own `propose_encounter` `INTENT`, not as a reply to this frame. |
...
3. **CORRECTED 2026-08-12 (§0).** As originally written this rule said "the module is the server, the agent is the client, and the agent reconnects" — backwards. `scripts/socket.js` runs as a client-side `esmodule` inside the GM's browser (`module.json` declares no server-side script); browsers have no API to listen for incoming connections, only to dial out. So it is the **agent** (a real Node process) that binds a listening socket, and the **module** that dials out and reconnects. Foundry is still the thing that must not be blocked by a missing peer — that constraint is what forces this direction once you know only one side of the pair can listen at all. Module reconnect: exponential backoff to a 10 s ceiling, and re-send `HELLO`.
