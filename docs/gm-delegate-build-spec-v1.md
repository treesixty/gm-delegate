...
| **TokenHUD DOM structure, resolved for the M3 `voiceNpc()` gap.** Live-inspected `canvas.hud.token.element.outerHTML`: root `<form id="token-hud" class="placeable-hud">`, buttons follow `.control-icon` + `data-action="..."` — the same pattern `panel.js`'s own `actions` map already uses. Not yet wired to `voiceNpc()`; this just confirms the seam (`renderTokenHUD` hook + an injected `.control-icon` button) is real and buildable whenever that gap is picked up. | 2026-08-12 | live console inspection, RunPod pod `d90mhv7i5kvqyg`, v14.365 | Low |
| Foundry exposes dedicated `combatTurn(combat, updateData: {round, turn}, updateOptions)` and `combatRound(combat, updateData, updateOptions)` and `combatStart(combat, updateData)` hooks — more precise than the generic `updateCombat` §4.6's `HOOKS` list uses, and these fire **before** the database update (so `updateData` has the new round/turn; `combat.round`/`combat.turn` on the document itself may still be stale at fire time, unlike `updateCombat`). **Not adopted** — §4.6 tags its `HOOKS` list `SHAPE — the hook list is normative`, so swapping is a spec change proposed here, not applied. Plausible explanation for a live `{round: 2, turn: null}` observed from `updateCombat` during an empty-tracker round advance. | 2026-08-12 | `foundryvtt.com/api/functions/hookEvents.combatTurn.html`; `.combatRound.html`; `.combatStart.html` | Medium |
